ggplot_dual_axis = function(plot1, plot2, which.axis = "x") {
  ...
  # Added border around the plot, so the secondary axis is not "flying around"
  
  # Update plot with transparent panel
  library(gridExtra)
  library(grid)
  plot2 = plot2 + theme(panel.background = element_rect(fill = NA))
  plot1 = plot1 + theme(panel.border = element_rect(colour = "black", fill = "NA", size = 2))
  ...
  if (which.axis == "x") {
    ...
    g = gtable_add_grob(g,grobs = rectGrob(gp = gpar(fill = NA,lwd = 4)),
                        t = 3,b = nrow(g) - 4,l = 4,r = ncol(g) - 2)
  }
  ## Draw it
  grid.newpage()
  grid.draw(g)  
