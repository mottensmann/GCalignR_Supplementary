...
df_bumblebee <- rbind(bbim,bfla,beph)
df_bumblebee[["id"]] <- rep(c("bbim","bfla","beph"),each = nrow(df)/3)
save(df_bumblebee, file = "data/df_bumblebee.RData")
```
...
df_bumblebee[["id"]] <- plyr::revalue(df_bumblebee[["id"]], c("bbim" = "Bombus bimaculatus", "beph" = "B. ephippiatus", "bfla" = "B. flavifrons"))
df_bumblebee[["id"]] <- factor(df_bumblebee[["id"]], levels = c("Bombus bimaculatus","B. ephippiatus", "B. flavifrons"))
# Figure 8
ggplot(df_bumblebee, aes(x = noise, y = error,group = id, col = id, fill = id)) + 
  facet_wrap(~id) +
...
plot <- ggplot(df, aes(x = Peak, y = diff, fill = data)) +
  ...
  labs(y = "Deviation from reference\nretention time [minutes]",
       x = "Cuticular hydrocarbon peak number") +
